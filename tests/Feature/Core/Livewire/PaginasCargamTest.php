...
test('las paginas de usuarios cargan con el layout de la app', function () {
    $administrador = User::factory()->create()->assignRole('administrador');
    $otroUsuario = User::factory()->create()->assignRole('cobrador');

    $this->actingAs($administrador)->get(route('usuarios.index'))->assertOk();
    $this->actingAs($administrador)->get(route('usuarios.crear'))->assertOk();
    $this->actingAs($administrador)->get(route('usuarios.editar', $otroUsuario))->assertOk();
});
